...
export const SUPPORTED_CHAINS: readonly Chain[] = ['TRC20', 'BEP20', 'ERC20', 'POLYGON', 'ARBITRUM'] as const

/** All supported tokens */
export const SUPPORTED_TOKENS: readonly Token[] = ['USDT', 'USDC'] as const

/**
 * Valid chain+token combinations.
 * TRC20 only supports USDT (Circle discontinued USDC on Tron).
 * All EVM chains support both USDT and USDC.
 */
export const VALID_CHAIN_TOKEN: Record<Chain, readonly Token[]> = {
  TRC20: ['USDT'],
  BEP20: ['USDT', 'USDC'],
  ERC20: ['USDT', 'USDC'],
  POLYGON: ['USDT', 'USDC'],
...
/** Check if a chain+token combination is valid */
export function isValidChainToken(chain: Chain, token: Token): boolean {
  return (VALID_CHAIN_TOKEN[chain] as readonly string[]).includes(token)
}
...
export const CHAIN_EXPLORER_TX: Record<Chain, string> = {
