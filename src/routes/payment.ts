...
export function createPaymentRouter(config: AppConfig): Router {
  ...
  /** Human-readable chain labels with fee hints for the chain selector */
  const CHAIN_SELECTOR_LABELS: Record<string, string> = {
    TRC20: 'TRON (TRC20)',
   ...
   * shows a chain selection page first.
   *
   * Query params:
   ...
   * - chain: TRC20, BEP20, ERC20, POLYGON, or ARBITRUM (optional, defaults to config)
  ...
  router.get('/create', async (req, res) => {
    ...
    // If multiple chains are enabled and customer hasn't selected one yet,
    // show the chain selector page before creating the payment.
    const showChainSelector = config.app.enabledChains.length > 1
    const chainSpecified = chain && isValidChain(chain)

    if (showChainSelector && !chainSpecified) {
      // Build the list of enabled chains with labels and supported tokens
      const enabledChainOptions = config.app.enabledChains.map(c => ({
        code: c,
        label: CHAIN_SELECTOR_LABELS[c] || CHAIN_LABELS[c] || c,
        tokens: VALID_CHAIN_TOKEN[c] as readonly string[],
      }))

      // Build base URL params to forward when the customer selects a chain
      ...
      res.render('chain-select', {
        enabledChains: enabledChainOptions,
    ...
    // Resolve chain: use query param if valid, otherwise fall back to config default
    const paymentChain: Chain = (chain && isValidChain(chain)) ? chain : config.app.defaultChain
    ...
    // Validate selected chain is in the enabled list
    if (!config.app.enabledChains.includes(paymentChain)) {
      res.status(400).render('error', {
        title: 'Invalid Network',
        message: `${paymentChain} is not enabled. Available networks: ${config.app.enabledChains.join(', ')}`,
        appUrl: config.app.url,
        texts: loadTexts(),
      })
      return
    ...
    // Validate chain+token combination (e.g. TRC20 only supports USDT)
    if (!isValidChainToken(paymentChain, paymentToken)) {
      res.status(400).render('error', {
        title: 'Invalid Configuration',
        message: `${paymentToken} is not supported on ${paymentChain}.`,
        appUrl: config.app.url,
        texts: loadTexts(),
      })
      return
  ...
  router.get('/:paymentId', async (req, res) => {
    ...
    res.render('payment', {
      ...
      chain: mapping.chain,
      network: CHAIN_LABELS[mapping.chain] || mapping.chain,
  ...
  router.get('/:paymentId/complete', async (req, res) => {
    ...
    const explorerBase = CHAIN_EXPLORER_TX[mapping.chain] || ''
