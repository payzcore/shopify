...
import { CHAIN_EXPLORER_TX } from '../services/payzcore'
import type { Chain } from '../services/payzcore'
...
async function handlePaymentSuccess(
  ...
  console.log(
    ...
    `(${payload.paid_amount} ${tokenLabel} on ${payload.chain})`
  )
  ...
  // Build explorer URL dynamically from chain
  const explorerBase = CHAIN_EXPLORER_TX[payload.chain as Chain]
  ...
  const note = [
    ...
    `Amount: ${payload.paid_amount} ${tokenLabel} (${payload.chain})`,
