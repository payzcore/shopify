...
<script>
  ...
  function payWithCrypto(chain, token) {
    ...
    // Optional: specify chain and token (defaults to app config if omitted)
    // If ENABLED_CHAINS has multiple chains and no chain param is passed,
    // the app will show a chain selector page to the customer.
    // Chains: TRC20, BEP20, ERC20, POLYGON, ARBITRUM
    // Tokens: USDT, USDC
...
| `DEFAULT_CHAIN` | No | Default blockchain network: `TRC20`, `BEP20`, `ERC20`, `POLYGON`, or `ARBITRUM` (default: `TRC20`) |
| `DEFAULT_TOKEN` | No | Default stablecoin token: `USDT` or `USDC` (default: `USDT`) |
| `ENABLED_CHAINS` | No | Comma-separated list of chains customers can choose from (e.g. `TRC20,BEP20,POLYGON`). If not set, only `DEFAULT_CHAIN` is available. When multiple chains are enabled, customers see a network selection page before paying. |
